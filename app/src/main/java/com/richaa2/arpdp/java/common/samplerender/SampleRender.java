 ...
 */
package com.richaa2.arpdp.java.common.samplerender;
...
public class SampleRender {
  ...
  private void useFramebuffer(Framebuffer framebuffer) {
    ...
    // Ensure we only bind a valid framebuffer ID; fallback to default 0 if not
    if (framebufferId != 0 && !GLES30.glIsFramebuffer(framebufferId)) {
      framebufferId = 0;
    }
